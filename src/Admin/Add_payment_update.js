...
function Add_payment_update() {
  ...
  //   axios.post('http://localhost/hospital/admin/add_payment.php', blogdata)
  ...
  const [amount, setamount] = useState("");
  const [discount, setdiscount] = useState("");
  const [paymet_method, setpayment_method] = useState("");

  // Fetch patient data if 'id' exists in the URL
  useEffect(() => {
    if (id) {
      axios
        .get(`http://localhost/hospital/admin/update_payment.php?id=${id}`)
        .then((response) => {
          if (response.data) {
  ...
  const handleSubmit = (e) => {
    ...
    formData.append("paymet_method", paymet_method);


    // Post the form data to update the patient
    axios
      .post("http://localhost/hospital/admin/update_payment1.php",formData)
      .then((response) => {
...
export default Add_payment_update;
