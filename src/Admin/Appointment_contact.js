...
function Appointment_contact () {
    const [blogdata,setblogdata] = useState([])
    useEffect (()=>{

        axios.post('http://localhost/hospital/admin/contact_select.php')
  ...
  .finally(function () {
    ...
    const handleDelete = (id) => {
      if (window.confirm("Are you sure you want to delete this item?")) {
          axios.post('http://localhost/hospital/admin/contact_Delete.php', { id })
              .then((response) => {
                        ...
                        <table className="table m-b-0 table-hover">
                          ...
                          <>
                          {
                            blogdata.map(jay=>
                                <tr>
                                <td>{jay.id}</td>
                                <td><span className="list-name">{jay.name}</span></td>
                                <td>{jay.email}</td>
                                <td>{jay.phone}</td>
                                <td>{jay.subject}</td>
                                <td>{jay.message}</td>
...
export default Appointment_contact;